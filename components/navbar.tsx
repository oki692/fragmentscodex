...
const SidebarToggleIcon = ({ size = 20 }: { size?: number }) => (
...
export function NavBar({
  ...
  return (
    <nav className="w-full flex items-center bg-white dark:bg-[#141414] py-3 md:py-4 sticky top-0 z-20 gap-1 md:gap-2">

      {/* Mobile: sidebar toggle + logo (left) */}
      <div className="flex md:hidden items-center gap-1 flex-shrink-0">
        {onToggleSidebar && (
          <button
            onClick={onToggleSidebar}
            className="flex items-center justify-center p-2 rounded-lg text-gray-500 dark:text-[#8A8A8D] hover:bg-gray-100 dark:hover:bg-[#2c2c2e] focus:outline-none active:outline-none"
      ...
      <div className="hidden md:flex flex-1 items-center gap-2">
        <Link href="/" className="flex items-center gap-2" target="_blank">
          ...
          <h1 className="whitespace-pre font-medium">Fragments</h1>
        </Link>
      </div>

      {/* Mobile pickers – center */}
      <div className="flex md:hidden flex-1 items-center justify-center gap-1 overflow-x-auto scrollbar-none min-w-0">
        {children}
      </div>
